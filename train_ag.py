...
parser.add_argument('--r', default=0.15, type=float, help='noise ratio')
parser.add_argument('--noise_mode',  default='idn')
...
parser.add_argument('--lambda_u', default=35, type=float, help='weight for unsupervised loss')
...
def show_time(time_difference):
    ...
    # 显示结果  
    print(f"操作所花费的时间为：{hours}小时 {minutes}分 {seconds}秒")
...
def test(epoch, net1, net2, test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets,length) in enumerate(test_loader):
            ...
            predicted_ = predicted.cpu().numpy().tolist()
            pred_list.extend(predicted_)

            targets_ = targets.cpu().numpy().tolist()
            tar_list.extend(targets_)
    ...
    #test_log.write('Epoch:%d   Accuracy:%.2f\n' % (epoch + 1, acc))
    test_log.flush()
    return acc_sum,pred_list,tar_list
...
# current_acc,pred_list,tar_list = test(0, net1, net2,test_loader)

# pred_file='/output/'+ args.dataset+'_'+args.noise_mode+str(args.r)+'_'+str(current_acc)+'_pred'+'.json'
# tar_file='/output/'+ args.dataset+'_'+args.noise_mode+str(args.r)+'_'+str(current_acc)+'_tar'+'.json'
# with open(pred_file, 'w') as file:
#     json.dump(pred_list, file)
# with open(tar_file, 'w') as file:
#     json.dump(tar_list, file)
...
for epoch in range (args.num_epochs):
    ...
    current_acc,pred_list,tar_list = test(epoch, net1, net2,test_loader)
