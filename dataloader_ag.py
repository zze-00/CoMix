...
class Translator:
    ...
    def __init__(self, path, transform_type='BackTranslation'):
        ...
        with open(path + '/ru_ag_5000.pkl', 'rb') as f:
...
class imdb_dataset(Dataset):
    def __init__(self, data_path, r, mode, noise_mode,noise_file='', model='/model/zhuoer/bert',max_seq_len=512,pred_sum=[],un_aug=False):
        ...
        if self.mode == 'test':
            test_df = pd.read_csv(data_path + '/test_ag_head.csv')
            ...
            train_df = pd.read_csv(data_path + '/IDN/idn_0.35.csv')  #
            ...
            #                 noiselabel_i = random.randint(0,3) 

            #             elif self.noise_mode== 'asym':
            #                 noiselabel_i = np.random.choice([train_labels[i], (train_labels[i] + 1) % 4], p=[1 - self.r, self.r])
