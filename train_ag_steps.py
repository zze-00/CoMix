...
import dataloader_ag as dataloader

from torch.utils.tensorboard import SummaryWriter

parser = argparse.ArgumentParser(description='AG Training')
parser.add_argument('--gpu', default='0,1,2,3', type=str)
parser.add_argument('--n_labels', default=4, type=int)
parser.add_argument('--mix_option', default=True, type=bool, help='mix option, whether to mix or not')
parser.add_argument('--data_path', default='/data/zhuoer/AG_news', type=str, help='path to dataset')
...
parser.add_argument('--lambda_u', default=20, type=float, help='weight for unsupervised loss')
...
steps = 0
writer = SummaryWriter(log_dir='/output/logs/ag_sym0.2_accuracy')
...
def warmup(epoch,net1,optimizer1,net2,optimizer2,dataloader,test_loader): #一个网络CE
    global steps
    net1.train()
    net2.train()
    ...
    for batch_idx, (inputs, labels, length) in enumerate(dataloader):
        ...
        loss1 = CEloss(outputs1, labels)   #cross entropy #SOFTMAX
        ...
        loss_total1 += loss1.item() * inputs.shape[0]
        total_sample1 += inputs.shape[0]

        outputs2 = net2(inputs)
        loss2 = CEloss(outputs2, labels)   #cross entropy #SOFTMAX
        ...
        correct2 += (np.array(predicted2.cpu()) ==
                    np.array(labels.cpu())).sum()
        loss_total2 += loss2.item() * inputs.shape[0]
        total_sample2 += inputs.shape[0]

        if (steps%100==0):
            test_steps(steps, net1, net2, test_loader)
...
def test_steps(steps, net1, net2, test_loader):
    net1.eval()
    net2.eval()
    ...
    writer.add_scalar('Test acc', acc_sum, steps)

    # print("\n| Test:Steps #%d\t Accuracy1: %.2f%%" % (steps, acc1))
    # print("| Test:Steps #%d\t Accuracy2: %.2f%%" % (steps, acc2))
    # print("| Test:Steps #%d\t Accuracy: %.2f%%" % (steps, acc_sum))
    return acc_sum
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader,test_loader):
    global steps
...
loader = dataloader.imdb_dataloader(data_path=args.data_path,r=args.r,noise_mode=args.noise_mode,batch_size=args.batch_size,num_workers=16,\
    noise_file='/output/noise_ratio= %.1f_%s.json'%(args.r,args.noise_mode))
...
for epoch in range (args.num_epochs):
    ...
    if epoch < args.warm_up:
        ...
        print('Warmup Net')
        warmup(epoch, net1, optimizer1, net2, optimizer2,warmup_trainloader,test_loader)
        ...
        train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader,test_loader)  # train net1

        print('\nTrain Net2')
        labeled_trainloader, unlabeled_trainloader = loader.run('train', pred_sum1,args.un_aug)
        train(epoch, net2, net1, optimizer2, labeled_trainloader, unlabeled_trainloader,test_loader)  # train net2
        ###warm up的时候要aug吗？试一下
