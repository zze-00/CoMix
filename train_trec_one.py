...
import dataloader_trec as dataloader


parser = argparse.ArgumentParser(description='TREC Training')
...
parser.add_argument('--data_path', default='./data/trec', type=str, help='path to dataset')
...
def train(epoch, net, optimizer, labeled_trainloader, unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x,  labels_x, _) in enumerate(labeled_trainloader):
        ...
        with torch.no_grad():
            ...
            pu = (torch.softmax(outputs_u1, dim=1)  + torch.softmax(outputs_u3, dim=1) )/ 2
