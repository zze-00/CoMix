...
parser.add_argument('--warm_up', default=15, type=int)
...
def test(epoch, net1,  test_loader):
    net1.eval()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets,length) in enumerate(test_loader):
            ...
            predicted1_ = predicted1.cpu().numpy().tolist()
            pred_list.extend(predicted1_)
    ...
    #test_log.write('Epoch:%d   Accuracy:%.2f\n' % (epoch + 1, acc))
    test_log.flush()
    return acc1,pred_list,tar_list
...
for epoch in range (args.num_epochs):
    ...
    current_acc,pred_list,tar_list = test(epoch, net1, test_loader)
