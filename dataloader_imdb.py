...
class Translator:
    ...
    def __init__(self, path, transform_type='BackTranslation'):
        ...
        with open(path + '/ru_2.pkl', 'rb') as f:
...
class imdb_dataset(Dataset):
    def __init__(self, data_path, r, mode, noise_mode,noise_file='', model='/model/zhuoer/bert',max_seq_len=512,pred_sum=[],un_aug=False):
        ...
        if self.mode == 'test':
            test_df = pd.read_csv("/data/zhuoer/IMDB/test_5000.csv")
            ...
            train_df = pd.read_csv("/data/zhuoer/IMDB/IDN/idn_0.15.csv")  #
            train_texts = np.array([v for v in train_df['review']])  #
            noise_labels = np.array([v for v in train_df['label']])  #
            ...
            #     noise_idx = idx[:num_noise]  # 有噪音的数据序号
                ...
                #             noiselabel_i = random.randint(0, 1)  # [0,1] 之间的任何数

                #         elif self.noise_mode== 'asym':
                #             noiselabel_i = np.random.choice([train_labels[i], (train_labels[i] + 1) % 2], p=[1 - self.r, self.r])
