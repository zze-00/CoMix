    ...
    def forward(self, hidden_states, hidden_states2=None, l=None, mix_layer=1000, attention_mask=None, attention_mask2=None, head_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            if i <= mix_layer:
                ...
                if self.output_hidden_states:  #bool 两边个算个的 hidden_states
                ...
                if self.output_attentions:  #bool
...
class MixText(nn.Module): #Tmix or not
    def __init__(self, num_labels=2, mix_option=False):
        super(MixText, self).__init__()

        if mix_option:
            self.bert = BertModel4Mix.from_pretrained('/model/zhuoer/bert')
        else:
            self.bert = BertModel.from_pretrained('/model/zhuoer/bert')
